import { Canvas, Description, Meta, Story } from "@storybook/addon-docs";
import { DesignTokenDocBlock } from "storybook-design-token/dist/doc-blocks";
import Procedure from "../index";
import pkg from "../../package.json";
import readme from "../../README.md";
import "../index.scss";

<Meta
  title="CSS/Data Display/Procedure"
  component={Procedure}
  parameters={{
    componentSubtitle: `${pkg.name} - ${pkg.version}`,
    status: {
      type: "WORK IN PROGRESS",
    },
  }}
/>

<Description markdown={readme} />

## Design Tokens

<DesignTokenDocBlock categoryName="Data Display" viewType="table" />

## Stories

### Default

<Canvas>
  <Story name="Default">
    <div className="denhaag-procedure-container">
      <ol className="denhaag-process-steps denhaag-procedure">
        <li className="denhaag-process-steps__step denhaag-process-steps__step--current">
          <div className="denhaag-process-steps__step-header">
            <div className="denhaag-step-marker denhaag-step-marker--current"></div>
            <p className="denhaag-process-steps__step-heading utrecht-heading-4">Maak een afspraak</p>
          </div>
          <div className="denhaag-process-steps__step-body">
            <div className="denhaag-step-marker__connector denhaag-step-marker__connector--current denhaag-step-marker__connector--main-to-nested"></div>
            <div className="denhaag-process-steps__step-meta">
              <p className="utrecht-paragraph">
                Voor het aanvragen van een nieuw paspoort moet u langskomen op Stadhuis Spui of stadskantoor Leyweg. U
                maakt daarvoor een afspraak.
              </p>
            </div>
          </div>
        </li>
        <li className="denhaag-process-steps__step denhaag-process-steps__step--current">
          <div className="denhaag-process-steps__step-header">
            <div className="denhaag-step-marker denhaag-step-marker--current"></div>
            <p className="denhaag-process-steps__step-heading utrecht-heading-4">
              Kom goed voorbereid naar de afspraak
            </p>
          </div>
          <div className="denhaag-process-steps__step-body">
            <div className="denhaag-step-marker__connector denhaag-step-marker__connector--current denhaag-step-marker__connector--main-to-nested"></div>
            <div className="denhaag-process-steps__step-meta">
              <p className="utrecht-paragraph">
                U heeft een aantal dingen nodig tijdens de afspraak.
                <br />
                <a
                  href="https://denhaag.nl"
                  className="denhaag-link denhaag-link--with-icon denhaag-link--with-icon-end denhaag-link--external"
                  rel="noreferrer noopener nofollow"
                >
                  <span className="denhaag-link__label">Bekijk wat u moet meenemen</span>
                  <span className="denhaag-link__sr-only">External link</span>
                  <svg
                    width="1em"
                    height="1em"
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                    className="denhaag-icon"
                    focusable="false"
                    aria-hidden="true"
                    shapeRendering="auto"
                  >
                    <path
                      d="M14 5C13.4477 5 13 4.55228 13 4C13 3.44772 13.4477 3 14 3H20C20.2652 3 20.5196 3.10536 20.7071 3.29289C20.8946 3.48043 21 3.73478 21 4L21 10C21 10.5523 20.5523 11 20 11C19.4477 11 19 10.5523 19 10L19 6.41422L9.70711 15.7071C9.31658 16.0976 8.68342 16.0976 8.29289 15.7071C7.90237 15.3166 7.90237 14.6834 8.29289 14.2929L17.5858 5H14ZM3 7C3 5.89543 3.89543 5 5 5H10C10.5523 5 11 5.44772 11 6C11 6.55228 10.5523 7 10 7H5V19H17V14C17 13.4477 17.4477 13 18 13C18.5523 13 19 13.4477 19 14V19C19 20.1046 18.1046 21 17 21H5C3.89543 21 3 20.1046 3 19V7Z"
                      fill="currentColor"
                    ></path>
                  </svg>
                </a>
              </p>
            </div>
          </div>
        </li>
        <li className="denhaag-process-steps__step denhaag-process-steps__step--current">
          <div className="denhaag-process-steps__step-header">
            <div className="denhaag-step-marker denhaag-step-marker--current"></div>
            <p className="denhaag-process-steps__step-heading utrecht-heading-4">Haal uw paspoort op</p>
          </div>
          <div className="denhaag-process-steps__step-body">
            <div className="denhaag-step-marker__connector denhaag-step-marker__connector--current denhaag-step-marker__connector--main-to-nested"></div>
            <div className="denhaag-process-steps__step-meta">
              <p className="utrecht-paragraph">
                Na 5 werkdagen is uw paspoort klaar en kunt u het ophalen.
                <br />
                <a href="#example-link" className="denhaag-link">
                  <span className="denhaag-link__label">Bekijk wat u nodig heeft bij het ophalen</span>
                </a>
              </p>
            </div>
          </div>
        </li>
      </ol>
    </div>
  </Story>
</Canvas>

### Small

<Canvas>
  <Story name="Small">
    <div className="denhaag-procedure-container" style={{ maxWidth: "616px" }}>
      <ol className="denhaag-process-steps denhaag-procedure">
        <li className="denhaag-process-steps__step denhaag-process-steps__step--current">
          <div className="denhaag-process-steps__step-header">
            <div className="denhaag-step-marker denhaag-step-marker--current"></div>
            <p className="denhaag-process-steps__step-heading utrecht-heading-4">Maak een afspraak</p>
          </div>
          <div className="denhaag-process-steps__step-body">
            <div className="denhaag-step-marker__connector denhaag-step-marker__connector--current denhaag-step-marker__connector--main-to-nested"></div>
            <div className="denhaag-process-steps__step-meta">
              <p className="utrecht-paragraph">
                Voor het aanvragen van een nieuw paspoort moet u langskomen op Stadhuis Spui of stadskantoor Leyweg. U
                maakt daarvoor een afspraak.
              </p>
            </div>
          </div>
        </li>
        <li className="denhaag-process-steps__step denhaag-process-steps__step--current">
          <div className="denhaag-process-steps__step-header">
            <div className="denhaag-step-marker denhaag-step-marker--current"></div>
            <p className="denhaag-process-steps__step-heading utrecht-heading-4">
              Kom goed voorbereid naar de afspraak
            </p>
          </div>
          <div className="denhaag-process-steps__step-body">
            <div className="denhaag-step-marker__connector denhaag-step-marker__connector--current denhaag-step-marker__connector--main-to-nested"></div>
            <div className="denhaag-process-steps__step-meta">
              <p className="utrecht-paragraph">
                U heeft een aantal dingen nodig tijdens de afspraak.
                <br />
                <a
                  href="https://denhaag.nl"
                  className="denhaag-link denhaag-link--with-icon denhaag-link--with-icon-end denhaag-link--external"
                  rel="noreferrer noopener nofollow"
                >
                  <span className="denhaag-link__label">Bekijk wat u moet meenemen</span>
                  <span className="denhaag-link__sr-only">External link</span>
                  <svg
                    width="1em"
                    height="1em"
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                    className="denhaag-icon"
                    focusable="false"
                    aria-hidden="true"
                    shapeRendering="auto"
                  >
                    <path
                      d="M14 5C13.4477 5 13 4.55228 13 4C13 3.44772 13.4477 3 14 3H20C20.2652 3 20.5196 3.10536 20.7071 3.29289C20.8946 3.48043 21 3.73478 21 4L21 10C21 10.5523 20.5523 11 20 11C19.4477 11 19 10.5523 19 10L19 6.41422L9.70711 15.7071C9.31658 16.0976 8.68342 16.0976 8.29289 15.7071C7.90237 15.3166 7.90237 14.6834 8.29289 14.2929L17.5858 5H14ZM3 7C3 5.89543 3.89543 5 5 5H10C10.5523 5 11 5.44772 11 6C11 6.55228 10.5523 7 10 7H5V19H17V14C17 13.4477 17.4477 13 18 13C18.5523 13 19 13.4477 19 14V19C19 20.1046 18.1046 21 17 21H5C3.89543 21 3 20.1046 3 19V7Z"
                      fill="currentColor"
                    ></path>
                  </svg>
                </a>
              </p>
            </div>
          </div>
        </li>
        <li className="denhaag-process-steps__step denhaag-process-steps__step--current">
          <div className="denhaag-process-steps__step-header">
            <div className="denhaag-step-marker denhaag-step-marker--current"></div>
            <p className="denhaag-process-steps__step-heading utrecht-heading-4">Haal uw paspoort op</p>
          </div>
          <div className="denhaag-process-steps__step-body">
            <div className="denhaag-step-marker__connector denhaag-step-marker__connector--current denhaag-step-marker__connector--main-to-nested"></div>
            <div className="denhaag-process-steps__step-meta">
              <p className="utrecht-paragraph">
                Na 5 werkdagen is uw paspoort klaar en kunt u het ophalen.
                <br />
                <a href="#example-link" className="denhaag-link">
                  <span className="denhaag-link__label">Bekijk wat u nodig heeft bij het ophalen</span>
                </a>
              </p>
            </div>
          </div>
        </li>
      </ol>
    </div>
  </Story>
</Canvas>

### Long Content

<Canvas>
  <Story name="Long Content">
    <div className="denhaag-procedure-container" style={{ maxWidth: "616px" }}>
      <ol className="denhaag-process-steps denhaag-procedure">
        <li className="denhaag-process-steps__step denhaag-process-steps__step--current">
          <div className="denhaag-process-steps__step-header">
            <div className="denhaag-step-marker denhaag-step-marker--current"></div>
            <p className="denhaag-process-steps__step-heading utrecht-heading-4">Kom in contact</p>
          </div>
          <div className="denhaag-process-steps__step-body">
            <div className="denhaag-step-marker__connector denhaag-step-marker__connector--current denhaag-step-marker__connector--main-to-nested"></div>
            <div className="denhaag-process-steps__step-meta">
              <p className="utrecht-paragraph">
                Bel (070) 353 8008 voor een gratis afspraak. U kunt kiezen voor een telefoongesprek of langskomen bij
                Helpdesk Geldzaken.
              </p>
            </div>
          </div>
        </li>
        <li className="denhaag-process-steps__step denhaag-process-steps__step--current">
          <div className="denhaag-process-steps__step-header">
            <div className="denhaag-step-marker denhaag-step-marker--current"></div>
            <p className="denhaag-process-steps__step-heading utrecht-heading-4">
              Kom goed voorbereid naar de afspraak
            </p>
          </div>
          <div className="denhaag-process-steps__step-body">
            <div className="denhaag-step-marker__connector denhaag-step-marker__connector--current denhaag-step-marker__connector--main-to-nested"></div>
            <div className="denhaag-process-steps__step-meta">
              <p className="utrecht-paragraph">
                Sed finibus vitae enim tincidunt pulvinar. Suspendisse rutrum ut erat maximus auctor. Aliquam vitae nibh
                bibendum, mattis tortor et, semper lectus. Ut tristique porttitor erat, sed eleifend odio elementum nec.
                Sed blandit nulla nulla, eu efficitur justo tempus vitae. Quisque feugiat lectus eget semper sagittis.
                In dignissim quis tellus vitae finibus. Cras faucibus augue eros, eu commodo tortor convallis at. Mauris
                ac egestas ipsum. Duis aliquet blandit sem eu interdum. Nullam eget odio non metus ornare rutrum. Mauris
                auctor purus ut tempus ultrices. Aenean at ex id quam tempor placerat. Sed ipsum libero, efficitur a
                mollis eget, scelerisque eget velit. Nulla condimentum nunc in ligula laoreet, quis vestibulum tortor
                luctus. In vitae leo lobortis, aliquet sem vitae, gravida purus. <br />
                <a
                  href="https://denhaag.nl"
                  className="denhaag-link denhaag-link--with-icon denhaag-link--with-icon-end denhaag-link--external"
                  rel="noreferrer noopener nofollow"
                >
                  <span className="denhaag-link__label">Bekijk wat u moet meenemen</span>
                  <span className="denhaag-link__sr-only">External link</span>
                  <svg
                    width="1em"
                    height="1em"
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                    className="denhaag-icon"
                    focusable="false"
                    aria-hidden="true"
                    shapeRendering="auto"
                  >
                    <path
                      d="M14 5C13.4477 5 13 4.55228 13 4C13 3.44772 13.4477 3 14 3H20C20.2652 3 20.5196 3.10536 20.7071 3.29289C20.8946 3.48043 21 3.73478 21 4L21 10C21 10.5523 20.5523 11 20 11C19.4477 11 19 10.5523 19 10L19 6.41422L9.70711 15.7071C9.31658 16.0976 8.68342 16.0976 8.29289 15.7071C7.90237 15.3166 7.90237 14.6834 8.29289 14.2929L17.5858 5H14ZM3 7C3 5.89543 3.89543 5 5 5H10C10.5523 5 11 5.44772 11 6C11 6.55228 10.5523 7 10 7H5V19H17V14C17 13.4477 17.4477 13 18 13C18.5523 13 19 13.4477 19 14V19C19 20.1046 18.1046 21 17 21H5C3.89543 21 3 20.1046 3 19V7Z"
                      fill="currentColor"
                    ></path>
                  </svg>
                </a>
              </p>
            </div>
          </div>
        </li>
        <li className="denhaag-process-steps__step denhaag-process-steps__step--current">
          <div className="denhaag-process-steps__step-header">
            <div className="denhaag-step-marker denhaag-step-marker--current"></div>
            <p className="denhaag-process-steps__step-heading utrecht-heading-4">
              Neque porro quisquam est qui dolorem ipsum quia dolor sit amet, consectetur, adipisci velit.
            </p>
          </div>
          <div className="denhaag-process-steps__step-body">
            <div className="denhaag-step-marker__connector denhaag-step-marker__connector--current denhaag-step-marker__connector--main-to-nested"></div>
            <div className="denhaag-process-steps__step-meta">
              <p className="utrecht-paragraph">
                Sed tempus sapien vel libero ultrices, aliquam convallis justo lacinia. Nam quis leo metus. Cras luctus
                vulputate consectetur. Vivamus convallis hendrerit mattis. Suspendisse quis placerat massa. Pellentesque
                vestibulum luctus dictum. Phasellus nec tortor suscipit lectus tincidunt convallis. Duis et euismod
                magna. Phasellus ultricies lacus quis convallis malesuada. Nullam a tellus faucibus dolor hendrerit
                viverra eu eu erat. Proin quis ultrices neque. Mauris pharetra erat non urna lobortis pretium.
                <br />
                <a href="#example-link" className="denhaag-link">
                  <span className="denhaag-link__label">Bekijk wat u nodig heeft bij het ophalen</span>
                </a>
              </p>
            </div>
          </div>
        </li>
      </ol>
    </div>
  </Story>
</Canvas>

### With heading

<Canvas>
  <Story name="With heading">
    <h2 className="utrecht-heading-2">Procedure:</h2>
    <div className="denhaag-procedure-container">
      <ol className="denhaag-process-steps denhaag-procedure">
        <li className="denhaag-process-steps__step denhaag-process-steps__step--current">
          <div className="denhaag-process-steps__step-header">
            <div className="denhaag-step-marker denhaag-step-marker--current"></div>
            <p className="denhaag-process-steps__step-heading utrecht-heading-4">Maak een afspraak</p>
          </div>
          <div className="denhaag-process-steps__step-body">
            <div className="denhaag-step-marker__connector denhaag-step-marker__connector--current denhaag-step-marker__connector--main-to-nested"></div>
            <div className="denhaag-process-steps__step-meta">
              <p className="utrecht-paragraph">
                Voor het aanvragen van een nieuw paspoort moet u langskomen op Stadhuis Spui of stadskantoor Leyweg. U
                maakt daarvoor een afspraak.
              </p>
            </div>
          </div>
        </li>
        <li className="denhaag-process-steps__step denhaag-process-steps__step--current">
          <div className="denhaag-process-steps__step-header">
            <div className="denhaag-step-marker denhaag-step-marker--current"></div>
            <p className="denhaag-process-steps__step-heading utrecht-heading-4">
              Kom goed voorbereid naar de afspraak
            </p>
          </div>
          <div className="denhaag-process-steps__step-body">
            <div className="denhaag-step-marker__connector denhaag-step-marker__connector--current denhaag-step-marker__connector--main-to-nested"></div>
            <div className="denhaag-process-steps__step-meta">
              <p className="utrecht-paragraph">
                U heeft een aantal dingen nodig tijdens de afspraak.
                <br />
                <a
                  href="https://denhaag.nl"
                  className="denhaag-link denhaag-link--with-icon denhaag-link--with-icon-end denhaag-link--external"
                  rel="noreferrer noopener nofollow"
                >
                  <span className="denhaag-link__label">Bekijk wat u moet meenemen</span>
                  <span className="denhaag-link__sr-only">External link</span>
                  <svg
                    width="1em"
                    height="1em"
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                    className="denhaag-icon"
                    focusable="false"
                    aria-hidden="true"
                    shapeRendering="auto"
                  >
                    <path
                      d="M14 5C13.4477 5 13 4.55228 13 4C13 3.44772 13.4477 3 14 3H20C20.2652 3 20.5196 3.10536 20.7071 3.29289C20.8946 3.48043 21 3.73478 21 4L21 10C21 10.5523 20.5523 11 20 11C19.4477 11 19 10.5523 19 10L19 6.41422L9.70711 15.7071C9.31658 16.0976 8.68342 16.0976 8.29289 15.7071C7.90237 15.3166 7.90237 14.6834 8.29289 14.2929L17.5858 5H14ZM3 7C3 5.89543 3.89543 5 5 5H10C10.5523 5 11 5.44772 11 6C11 6.55228 10.5523 7 10 7H5V19H17V14C17 13.4477 17.4477 13 18 13C18.5523 13 19 13.4477 19 14V19C19 20.1046 18.1046 21 17 21H5C3.89543 21 3 20.1046 3 19V7Z"
                      fill="currentColor"
                    ></path>
                  </svg>
                </a>
              </p>
            </div>
          </div>
        </li>
        <li className="denhaag-process-steps__step denhaag-process-steps__step--current">
          <div className="denhaag-process-steps__step-header">
            <div className="denhaag-step-marker denhaag-step-marker--current"></div>
            <p className="denhaag-process-steps__step-heading utrecht-heading-4">Haal uw paspoort op</p>
          </div>
          <div className="denhaag-process-steps__step-body">
            <div className="denhaag-step-marker__connector denhaag-step-marker__connector--current denhaag-step-marker__connector--main-to-nested"></div>
            <div className="denhaag-process-steps__step-meta">
              <p className="utrecht-paragraph">
                Na 5 werkdagen is uw paspoort klaar en kunt u het ophalen.
                <br />
                <a href="#example-link" className="denhaag-link">
                  <span className="denhaag-link__label">Bekijk wat u nodig heeft bij het ophalen</span>
                </a>
              </p>
            </div>
          </div>
        </li>
      </ol>
    </div>
  </Story>
</Canvas>

### Under paragraph

<Canvas>
  <Story name="Under paragraph">
    <p className="utrecht-paragraph">Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
    <div className="denhaag-procedure-container">
      <ol className="denhaag-process-steps denhaag-procedure">
        <li className="denhaag-process-steps__step denhaag-process-steps__step--current">
          <div className="denhaag-process-steps__step-header">
            <div className="denhaag-step-marker denhaag-step-marker--current"></div>
            <p className="denhaag-process-steps__step-heading utrecht-heading-4">Maak een afspraak</p>
          </div>
          <div className="denhaag-process-steps__step-body">
            <div className="denhaag-step-marker__connector denhaag-step-marker__connector--current denhaag-step-marker__connector--main-to-nested"></div>
            <div className="denhaag-process-steps__step-meta">
              <p className="utrecht-paragraph">
                Voor het aanvragen van een nieuw paspoort moet u langskomen op Stadhuis Spui of stadskantoor Leyweg. U
                maakt daarvoor een afspraak.
              </p>
            </div>
          </div>
        </li>
        <li className="denhaag-process-steps__step denhaag-process-steps__step--current">
          <div className="denhaag-process-steps__step-header">
            <div className="denhaag-step-marker denhaag-step-marker--current"></div>
            <p className="denhaag-process-steps__step-heading utrecht-heading-4">
              Kom goed voorbereid naar de afspraak
            </p>
          </div>
          <div className="denhaag-process-steps__step-body">
            <div className="denhaag-step-marker__connector denhaag-step-marker__connector--current denhaag-step-marker__connector--main-to-nested"></div>
            <div className="denhaag-process-steps__step-meta">
              <p className="utrecht-paragraph">
                U heeft een aantal dingen nodig tijdens de afspraak.
                <br />
                <a
                  href="https://denhaag.nl"
                  className="denhaag-link denhaag-link--with-icon denhaag-link--with-icon-end denhaag-link--external"
                  rel="noreferrer noopener nofollow"
                >
                  <span className="denhaag-link__label">Bekijk wat u moet meenemen</span>
                  <span className="denhaag-link__sr-only">External link</span>
                  <svg
                    width="1em"
                    height="1em"
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                    className="denhaag-icon"
                    focusable="false"
                    aria-hidden="true"
                    shapeRendering="auto"
                  >
                    <path
                      d="M14 5C13.4477 5 13 4.55228 13 4C13 3.44772 13.4477 3 14 3H20C20.2652 3 20.5196 3.10536 20.7071 3.29289C20.8946 3.48043 21 3.73478 21 4L21 10C21 10.5523 20.5523 11 20 11C19.4477 11 19 10.5523 19 10L19 6.41422L9.70711 15.7071C9.31658 16.0976 8.68342 16.0976 8.29289 15.7071C7.90237 15.3166 7.90237 14.6834 8.29289 14.2929L17.5858 5H14ZM3 7C3 5.89543 3.89543 5 5 5H10C10.5523 5 11 5.44772 11 6C11 6.55228 10.5523 7 10 7H5V19H17V14C17 13.4477 17.4477 13 18 13C18.5523 13 19 13.4477 19 14V19C19 20.1046 18.1046 21 17 21H5C3.89543 21 3 20.1046 3 19V7Z"
                      fill="currentColor"
                    ></path>
                  </svg>
                </a>
              </p>
            </div>
          </div>
        </li>
        <li className="denhaag-process-steps__step denhaag-process-steps__step--current">
          <div className="denhaag-process-steps__step-header">
            <div className="denhaag-step-marker denhaag-step-marker--current"></div>
            <p className="denhaag-process-steps__step-heading utrecht-heading-4">Haal uw paspoort op</p>
          </div>
          <div className="denhaag-process-steps__step-body">
            <div className="denhaag-step-marker__connector denhaag-step-marker__connector--current denhaag-step-marker__connector--main-to-nested"></div>
            <div className="denhaag-process-steps__step-meta">
              <p className="utrecht-paragraph">
                Na 5 werkdagen is uw paspoort klaar en kunt u het ophalen.
                <br />
                <a href="#example-link" className="denhaag-link">
                  <span className="denhaag-link__label">Bekijk wat u nodig heeft bij het ophalen</span>
                </a>
              </p>
            </div>
          </div>
        </li>
      </ol>
    </div>
  </Story>
</Canvas>

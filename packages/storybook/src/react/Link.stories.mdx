import { ArgsTable, Canvas, Description, Meta, Story } from "@storybook/blocks";
import { ArrowLeftIcon, ArrowRightIcon, Paragraph, Link } from "@gemeente-denhaag/components-react";

import pkg from "../../../../components/Link/package.json";
import readme from "../../../../components/Link/README.md";

<Meta
  title="React/Navigation/Link"
  component={Link}
  parameters={{
    componentSubtitle: `${pkg.name} - ${pkg.version}`,
    status: {
      type: "WORK IN PROGRESS",
    },
  }}
/>

<Description markdown={readme} />

## Usage

This component can be used in two ways:

- As a standard `a`-tag with icon support. For this you need to supply at least an `href`, and an icon can be added in the `icon` prop:
  `<Link href="/home" icon={<ArrowRightIcon />} iconAlign="end">Home</Link>`

- With another way of routing, for example using `react-router-dom`. For this we have exposed the `component` property. When adding a `component` all props of this component will be usable on our `Link` component and will be passed through.
  An example with `react-router-dom`:

  ```
  import { Link, ArrowRightIcon } from '@gemeente-denhaag/components-react';
  import {
    BrowserRouter as Router,
    Switch,
    Route,
    Link as RouterLink,
  } from 'react-router-dom';

  export default function App() {
    return (
      <Router>
        <div>
          <nav>
            <ul>
              <li>
                 <Link component={RouterLink} to="/" icon={<ArrowRightIcon />}>Home</Link>
              </li>
              <li>
                 <Link component={RouterLink} to="/nl" hrefLang="nl">NL</Link>
              </li>
            </ul>
          </nav>
           <Switch>
              <Route path="/nl">
                <h1>NL home</h1>
              </Route>
              <Route path="/">
                <h1>Home</h1>
              </Route>
            </Switch>
        </div>
      </Router>
    );
  }
  ```

## Stories

### Default

<Canvas>
  <Story
    name="Default"
    args={{}}
    parameters={{
      docs: {
        source: {
          code: `
import { Link, Paragraph } from "@gemeente-denhaag/components-react";
const ReactParagraph = (
  <Paragraph>
    It is possible to put <Link href="#">a link</Link> in a Paragraph element.
  </Paragraph>
);
    `,
        },
      },
    }}
  >
    {(args) => (
      <Paragraph>
        It is possible to put{" "}
        <Link href="#" {...args}>
          a link
        </Link>{" "}
        in a Paragraph element.
      </Paragraph>
    )}
  </Story>
</Canvas>

<ArgsTable of={Link} />

### Disabled

<Canvas>
  <Story
    name="Disabled"
    args={{ disabled: true }}
    parameters={{
      docs: {
        source: {
          code: `
import { Link, Paragraph } from "@gemeente-denhaag/components-react";
const ReactParagraph = (
  <Paragraph>
    It is possible to put <Link href="#" disabled>a link</Link> in a Paragraph element.
  </Paragraph>
);
    `,
        },
      },
    }}
  >
    {(args) => (
      <Paragraph>
        It is possible to put{" "}
        <Link href="#" {...args}>
          a link
        </Link>{" "}
        in a Paragraph element.
      </Paragraph>
    )}
  </Story>
</Canvas>

## Icon Start

<Canvas>
  <Story
    name="Icon Start"
    args={{ icon: <ArrowLeftIcon />, iconAlign: "start" }}
    parameters={{
      docs: {
        source: {
          code: `
import { Link, Paragraph, ArrowLeftIcon } from "@gemeente-denhaag/components-react";
const ReactParagraph = (
  <Paragraph>
    It is possible to put <Link href="#" icon={<ArrowLeftIcon />} iconAlign="start">a link</Link> in a Paragraph element.
  </Paragraph>
);
    `,
        },
      },
    }}
  >
    {(args) => (
      <Paragraph>
        It is possible to put{" "}
        <Link href="#" {...args}>
          a link
        </Link>{" "}
        in a Paragraph element.
      </Paragraph>
    )}
  </Story>
</Canvas>

### Icon End

<Canvas>
  <Story
    name="Icon End"
    args={{ icon: <ArrowRightIcon /> }}
    parameters={{
      docs: {
        source: {
          code: `
import { ArrowRightIcon, Link, Paragraph } from "@gemeente-denhaag/components-react";
const ReactParagraph = (
  <Paragraph>
    It is possible to put <Link href="#" icon={<ArrowRightIcon />}>a link</Link> in a Paragraph element.
  </Paragraph>
);
    `,
        },
      },
    }}
  >
    {(args) => (
      <Paragraph>
        It is possible to put{" "}
        <Link href="#" {...args}>
          a link
        </Link>{" "}
        in a Paragraph element.
      </Paragraph>
    )}
  </Story>
</Canvas>
